/****************************************************************************
** Meta object code from reading C++ file 'crop.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "crop.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'crop.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Crop_t {
    QByteArrayData data[7];
    char stringdata0[71];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Crop_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Crop_t qt_meta_stringdata_Crop = {
    {
QT_MOC_LITERAL(0, 0, 4), // "Crop"
QT_MOC_LITERAL(1, 5, 16), // "croppingFinished"
QT_MOC_LITERAL(2, 22, 0), // ""
QT_MOC_LITERAL(3, 23, 14), // "croppingClosed"
QT_MOC_LITERAL(4, 38, 8), // "finished"
QT_MOC_LITERAL(5, 47, 4), // "quit"
QT_MOC_LITERAL(6, 52, 18) // "setDefaultCropping"

    },
    "Crop\0croppingFinished\0\0croppingClosed\0"
    "finished\0quit\0setDefaultCropping"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Crop[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   39,    2, 0x06 /* Public */,
       3,    0,   40,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       4,    0,   41,    2, 0x0a /* Public */,
       5,    0,   42,    2, 0x0a /* Public */,
       6,    0,   43,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void Crop::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Crop *_t = static_cast<Crop *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->croppingFinished(); break;
        case 1: _t->croppingClosed(); break;
        case 2: _t->finished(); break;
        case 3: _t->quit(); break;
        case 4: _t->setDefaultCropping(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (Crop::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Crop::croppingFinished)) {
                *result = 0;
            }
        }
        {
            typedef void (Crop::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Crop::croppingClosed)) {
                *result = 1;
            }
        }
    }
    Q_UNUSED(_a);
}

const QMetaObject Crop::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_Crop.data,
      qt_meta_data_Crop,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *Crop::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Crop::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_Crop.stringdata0))
        return static_cast<void*>(const_cast< Crop*>(this));
    return QDialog::qt_metacast(_clname);
}

int Crop::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void Crop::croppingFinished()
{
    QMetaObject::activate(this, &staticMetaObject, 0, Q_NULLPTR);
}

// SIGNAL 1
void Crop::croppingClosed()
{
    QMetaObject::activate(this, &staticMetaObject, 1, Q_NULLPTR);
}
QT_END_MOC_NAMESPACE
