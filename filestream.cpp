...
Filestream::Filestream(std::string filename, int _width, int _height, int _offsetLeft, int _offsetRight, int _offsetTop, int _offsetBottom)
{
    isCropped = false;
    width = _width;
    height = _height;
    ...
    loadFromRawFile(filename.c_str(), width * height, data);
...
void Filestream::loadFromImage()
{
    ...
    cv::Mat img = cv::imread(full_path.toStdString() + "/h_or.jpg",0);
